...
:run_sbebatch
	...
	echo -------- Settings --------
	echo raw dir      : %path_data%
	echo raw suffix   : "%data_suffix%"
	...
	for %%p in (%path_data%*.%data_suffix%) do (
		...
		REM 	if %do_run_window_filtering% equ 0 (
		REM 		pause
		REM 	)
		REM 	exit /B
		REM )
	...
	echo DONE
	REM only ask for final user key press when this was the final step
	if %do_run_window_filtering% == 0 ( 
		pause
	)
goto:eof
