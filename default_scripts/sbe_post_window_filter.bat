...
:run_window_filter
	...
	echo -------- Settings --------
	echo proc dir      : %path_out%
	echo psa files     : %path_psa%
	echo raw data      : %path_data%
	...
	for %%p in (%path_data%*.%data_suffix%%) do (
		...
		if !do_continue! equ 1 (
			...
			set file_name=!prefix_name:.%data_suffix%=!
	...
	:: ask for press key in case this is not run together with general processing
	if %do_run_sbe_processing% neq 1 (
		pause
	)
	...
	for %%p in (%path_data%*.%data_suffix%) do (
		...
		if !do_this_file! equ 1 (
			...
			for  /f "tokens=*" %%x in ('dir /b %path_out%*!stn_num!".cnv" 2^>nul') do set set do_continue=0
			

			REM check if within station number bounds if specified
			REM end processing if above max
			REM if !stn_int! GTR !stn_max! (
			REM 	echo Stopped after last station in range: ^# %stn_max%
			REM 	pause
			REM 	exit /B
			REM )
			
			REM seems like processing not always starts with lowest to highest number, so have to skip above max number instead
			...
			if !do_continue! equ 1 (
				...
				echo  - window filter ^(short^) on temporary high resolution CNV file...
				sbebatch %relpath_sbebatch%sbebatch_window_filter_short.txt %path_psa% %path_out%!cnv_file! %path_out% !file_name!
				REM in this step the filename got appended by "_tmp" to not overwrite the original .cnv file
				...
				REM get _tmp.cnv filename
				for /F "tokens=* USEBACKQ" %%F in (`dir /b %path_out%*!stn_num!"_tmp.cnv"`) do (
					set cnv_tmp_file=%%F
				)
				
				REM average
				REM averaging over _tmp.cnv file
				echo  - averaging window filtered temporary CNV file...
				sbebatch %relpath_sbebatch%sbebatch_average_cdom_filter.txt %path_psa% %path_out%!cnv_tmp_file! %path_out% !file_name!
				REM this created a file ending on _bin_cdom.cnv so that the original _bin.cnv does not get overwritten
				
				REM delete temporary _tmp.cnv file again because we are now working on the _bin file
				echo  - removing temporary CNV file
				del %path_out%!cnv_tmp_file!
				
				REM window filter (long)
				REM get _bin.cnv filename
				for /F "tokens=* USEBACKQ" %%F in (`dir /b %path_out%*!stn_num!"_bin_cdom.cnv"`) do (
					set cnv_bin_file=%%F
				)
